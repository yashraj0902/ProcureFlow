...
from app.models.domain import Decision, TrackRecord

class DecisionInput(BaseModel):
    outcome: str
    rationale: str

@router.post("/{pilot_id}/decision")
def make_decision(pilot_id: int, decision: DecisionInput, db: Session = Depends(get_db)):
    pilot = db.query(Pilot).filter(Pilot.id == pilot_id).first()
    if not pilot:
        raise HTTPException(status_code=404, detail="Pilot not found")
    ...
    # Update Pilot Status
    pilot.status = "COMPLETED" if decision.outcome == "SCALE" else "CANCELLED" if decision.outcome == "STOP" else "ACTIVE"
    
    # Generate Verified Track Record if SCALE or STOP
    if decision.outcome in ["SCALE", "STOP"]:
        startup = db.query(Startup).filter(Startup.id == pilot.startup_id).first()
        challenge = db.query(Challenge).filter(Challenge.id == pilot.challenge_id).first()
        ...
        tr = TrackRecord(
            startup_id=pilot.startup_id,
            pilot_id=pilot.id,
            department_name="Ministry of Housing and Urban Affairs", # Mock
            challenge_title=challenge.title if challenge else "Unknown",
            achieved_results=f"Pilot outcome: {decision.outcome}. Rationale: {decision.rationale}",
